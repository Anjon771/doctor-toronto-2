...
export const SocialContentGrid: React.FC<SocialContentGridProps> = ({ onReadArticle }) => {
  ...
  return (
    <section id="updates-section" className="py-16 sm:py-24 bg-[#0A0A0E] relative border-t border-[#1C1F2E]">
            ...
            {tags.map((t) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                  selectedTag === t
                    ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30 ring-1 ring-cyan-300/40'
                    : 'bg-[#0E121E] text-slate-400 hover:text-slate-200 border border-slate-800'
                }`}
              >
                {t}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filteredArticles.map((article) => (
            <article
              ...
              className="group rounded-2xl bg-[#0E121E] border border-slate-800/90 hover:border-cyan-500/50 overflow-hidden flex flex-col justify-between transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-cyan-950/30 cursor-pointer"
            >
              {/* Image Banner */}
              <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-950">
                ...
                {/* Tag Badge */}
                <div className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-[#090D16]/90 backdrop-blur-md border border-cyan-500/30 text-[10px] font-bold text-cyan-300 shadow-md uppercase tracking-wider">
                  {article.tag}
                </div>

                {/* Read Time Tag */}
                <div className="absolute top-3 right-3 px-2.5 py-1 rounded-lg bg-black/85 text-[10px] font-medium text-slate-300 border border-white/10 flex items-center gap-1 backdrop-blur-sm">
              ...
              <div className="p-5 sm:p-6 flex flex-col justify-between flex-grow">
                ...
                <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs font-semibold text-cyan-400 group-hover:text-cyan-300">
                  ...
                  <span className="text-[11px] text-slate-500 group-hover:text-slate-400 font-mono uppercase">
                    Kasper Insights
                  </span>
                </div>
              </div>
